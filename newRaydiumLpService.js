...
const SniperManager = require('./SniperManager'); // Add this import
...
// ➕ ADD: Global lpData storage (or import from existing file if available)
const lpData = new Map();
    ...
    try {
        ...
        for (const ix of instructions) {
            ...
            if (programId === RAYDIUM_AMM_PROGRAM_ID.toString() && ix.data.length > 0) {
                ...
                // Save to MongoDB first (KEEP EXISTING - NO CHANGES)
                const insertedId = await saveToMongo(tokenData);
                const completeTokenData = { ...tokenData, _id: insertedId.toString() };

                // Store the  data in lpData Map for other files to access
                lpData.set(completeTokenData.ammId, completeTokenData);
                console.log(` Added to lpData: ${completeTokenData.ammId}`);
                console.log(` lpData now contains ${lpData.size} entries`);

                // 🔍 DEBUG: Check data right before passing to SniperManager
                console.log('=== PRE-SNIPER DEBUG ===');
                console.log('1. CompleteTokenData type:', typeof completeTokenData);
                console.log('2. Has ammId:', 'ammId' in completeTokenData);
                console.log('3. AmmId value:', completeTokenData.ammId);
                console.log('4. AmmId type:', typeof completeTokenData.ammId);
                console.log('5. Has tokenId:', 'tokenId' in completeTokenData);
                console.log('6. TokenId value:', completeTokenData.tokenId);
                console.log('7. lpData entry exists:', lpData.has(completeTokenData.ammId));
                console.log('8. All keys:', Object.keys(completeTokenData));

                // Test object structure
                const testRequiredFields = ['ammId', 'tokenId', 'baseMint', 'quoteMint', 'baseVault', 'quoteVault'];
                console.log('9. Required fields check:');
                testRequiredFields.forEach(field => {
                    console.log(`   ${field}: ${completeTokenData[field]} (${typeof completeTokenData[field]})`);
                });
                console.log('10. Calling SniperManager.addSniper with data...');
                console.log('=== END PRE-SNIPER DEBUG ===');

                // 🚀 Call SniperManager
                try {
                    await SniperManager.addSniper(completeTokenData);
                    console.log(`✅ Sniper launched successfully for ${completeTokenData.baseMint}`);
                } catch (sniperError) {
                    console.error(`❌ Sniper launch failed:`, sniperError.message);
                    console.error('Error stack:', sniperError.stack);
                }
...
// ➕ NEW: Export lpData for access by other modules
module.exports = {
    connectToDatabase,
    processRaydiumLpTransaction,
    lpData, // Export lpData so SniperManager can access it
};
